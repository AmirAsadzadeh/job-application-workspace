...
import type { ListViewPreference } from "@workspace/domain/positionSchema";

const dnd = vi.hoisted(() => ({ onDragEnd: undefined as undefined | ((event: unknown) => Promise<void>) }));
const browserActions = vi.hoisted(() => ({ savePackage: vi.fn(() => Promise.resolve(true)) }));
...
vi.mock("../../../browser/browserActions", () => ({ saveWorkspacePackage: browserActions.savePackage }));
