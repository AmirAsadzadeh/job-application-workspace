...
import { createLocalSynchronizationService, type LocalWorkspaceSelection } from "./localSynchronizationService.js";
import { createWorkspaceRepositoryFactory } from "./workspaceRepositoryFactory.js";
import { loadWorkspaceState } from "./workspaceModeState.js";
...
export function createApiHandler(repository: PositionsRepository, providedTransfer?: ReturnType<typeof createWorkspaceTransferService>, bundledLogoDirectoryPath?: string): ApiHandler {
  const dataDirectory = dirname(repository.paths.positionsPath);
  const transfer = providedTransfer ?? createWorkspaceTransferService({ repository, transferDirectoryPath: join(dataDirectory, ".workspace-transfer"), backupDirectoryPath: join(dataDirectory, "backups"), bundledLogoDirectoryPath: bundledLogoDirectoryPath ?? join(dirname(dataDirectory), "public", "company-logos") });
...
export async function startServer(options: StartServerOptions = {}) {
  ...
  const workspaceRoot = options.workspacePath ?? join(projectRoot, "data");
  ...
  const layout = await normalizeWorkspaceLayout(workspaceRoot);
  const dataDirectoryPath = layout.paths.root;
  ...
  async function activeContext() {
    ...
    const accountId = state.selectedMode === "online" ? state.selectedAccountId : null;
    const key = accountId ? `online:${accountId}` : "offline";
    const paths = accountId ? accountWorkspacePaths(workspaceRoot, accountId) : offlineWorkspacePaths(workspaceRoot);
    let handler = apiHandlers.get(key);
    if (!handler) {
      await initializeWorkspaceFiles(paths.root, templatePath);
      const repository = accountId ? repositories.online(accountId) : repositories.offline();
      handler = createApiHandler(repository, undefined, join(projectRoot, "public", "company-logos"));
      apiHandlers.set(key, handler);
    }
    return { handler, logoDirectoryPath: paths.logoDirectoryPath };
  }
  const modeHandler = createWorkspaceModeHandler(workspaceRoot);
  const localSynchronization = createLocalSynchronizationService(workspaceRoot, join(projectRoot, "public", "company-logos"));
  const localSynchronizationHandler = createLocalSynchronizationHandler(localSynchronization, async (target: LocalWorkspaceSelection) => {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (await active.handler(request, response)) return;
    ...
    if (pathname.startsWith("/company-logos/")) {
      ...
      if (await serveFile(active.logoDirectoryPath, logoName, response, true)) return;
